...
int main(int argc, char **argv){
    ...
    if(clientCommand(socketfd_control, command) == -1) return -1; 
    if(readResponse(socketfd_control, rd, sizeof(rd)) == -1) return -1; //response of opening mode DATA

    if(writeFile(socketfd_data, arguments.file_name) == -1) return -1;
    if(readResponse(socketfd_control, rd, sizeof(rd)) == -1) return -1; //response of finishing the transfer
